...
//  unsubiscribe 
 ...
 export const profilePic = async (req ,res, next) => {
    ...
    //finalDest => basic url for image 
    const user = await userModel.findByIdAndUpdate({ _id: req.user._id }, { profilPic: req.file.finalDest }, { new: true })
    return res.json({ messagge: "Done", file : req.file.finalDest })
 }
 ...
 export const coverPic = async (req,res,next) => {
    ...
    files.forEach(element => {
        image.push(element.finalDest)
    });
    const user = await userModel.findByIdAndUpdate({_id : req.user._id }, { coverPic: image} , {new : true })
    return res.json({ messagge: "Done", file : image  })
 }
