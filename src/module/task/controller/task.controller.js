...
export const attachment = async (req, res, next ) => {
  ...
  const task = await taskModel.findByIdAndUpdate({userID: req.user._id} , {attachmentTask : req.file.finalDest} , {new : true })
  return res.json({message : "done" , file : req.file.finalDest})
}